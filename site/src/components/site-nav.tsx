...
import { WEB_CAJA_PUENTE_URL } from "@/config/fondo";
...
type NavStrings = {
  ...
  faq: string;
  aportar: string;
  /** Tooltip del botón Aportar mientras la caja se segrega a su propio sitio. */
  aportarPronto: string;
  marca: string;
  menu: string;
  close: string;
...
const SECCION_A_EPIGRAFE: Record<string, string> = {
  conflicto: "conflicto",
  cronologia: "conflicto",
  termometro: "conflicto",
...
export function SiteNav({
  ...
  lang: string;
  nav: NavStrings;
  pagina?: "portada" | "documentacion" | "sobre";
  ...
  const links = [
    { href: ancla("conflicto"), id: "conflicto", label: nav.conflicto },
    { href: ancla("internacional"), id: "internacional", label: nav.internacional },
    { href: `/${lang}/documentacion`, id: "documentacion", label: nav.documentacion },
    { href: `/${lang}/sobre-el-proyecto`, id: "sobre", label: nav.sobre },
    ...
    <header className="sticky top-0 z-40 border-b border-[var(--color-linea)] bg-[color-mix(in_srgb,var(--color-fondo)_88%,transparent)] backdrop-blur-md">
      ...
      >
        ...
        <div className="hidden items-center gap-6 xl:flex">
          ...
          {/* «Aportar» — PUENTE TEMPORAL (3-sep-2026): enlaza a la página puente en GitHub Pages
              mientras los legales revisan Web B. Al activar Web B: volver al `<span>` inerte con
              `nav.aportarPronto`. */}
          <a
            href={WEB_CAJA_PUENTE_URL}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-block rounded-full bg-[var(--color-acento)] px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[var(--color-acento-hondo)]"
          >
            {nav.aportar}
          </a>
        </div>
      ...
      {open ? (
        <div id="mobile-menu" className="border-t border-[var(--color-linea)] xl:hidden">
          <ul className="mx-auto flex max-w-6xl flex-col gap-1 px-5 py-3 text-[var(--color-tinta)]">
            ...
            {/* «Aportar» — PUENTE TEMPORAL (3-sep-2026): enlaza a la página puente en GitHub Pages
                mientras los legales revisan Web B. Al activar Web B: volver al `<span>` inerte +
                la nota `nav.aportarPronto`. */}
            <li className="mt-2">
              <a
                href={WEB_CAJA_PUENTE_URL}
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => setOpen(false)}
                className="block rounded-full bg-[var(--color-acento)] px-5 py-3 text-center font-semibold text-white transition-colors hover:bg-[var(--color-acento-hondo)]"
              >
                {nav.aportar}
              </a>
            </li>
...
function LocaleSwitcher({
  ...
  const sufijo =
    pagina === "documentacion" ? "/documentacion"
    : pagina === "sobre" ? "/sobre-el-proyecto" : "";
  return (
